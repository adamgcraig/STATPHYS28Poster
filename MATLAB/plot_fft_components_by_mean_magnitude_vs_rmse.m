...
for subject_index = 1:num_subjects
    ...
    for ts_index = 1:ts_per_subject
        ...
        ts_file = get_time_series_file(subject_id, ts_string);
        % ts = std_mean_normalize_ts( load_time_series(ts_file) );
        ts = load_time_series(ts_file);
        ts_fft = fft(ts, num_time_points, 2);
        ts_fft_left = ts_fft(:,1:midpoint);
        ts_fft_abs = abs(ts_fft_left);
        [~, sort_indices] = sort(ts_fft_abs, 2, 'ascend');
        num_choices = size(sort_indices,2);
        for choice_index = 1:num_choices
            for area_index = 1:num_brain_areas
                freq_to_erase = sort_indices(area_index, choice_index);
                ts_fft(area_index, freq_to_erase) = 0;
                if freq_to_erase ~= 1
                    ts_fft(area_index, num_time_points - freq_to_erase + 2) = 0;
                end
            end
            ts_recon = ifft(ts_fft, num_time_points, 2, 'symmetric');
